// app/commodity/[slug]/HistoryChart.jsx
"use client";
...
export default function HistoryChart({ points }) {
  ...
  return (
    <div
      ref={ref}
      style={{ position: "relative", touchAction: "pan-y", cursor: "crosshair", marginTop: 6 }}
    ...
    >
      <svg viewBox={`0 0 ${W} ${Hh}`} width="100%" style={{ display: "block" }} role="img" aria-label="Price history chart — hover or drag to read values">
        ...
        {hi == null && (
          <g>
            ...
            <circle cx={X(lastI)} cy={Y(points[lastI][1])} r="3.5" fill={C.accent} />
          </g>
        )}

        {tip && (
          <g>
            <line x1={tip.px} y1={padT} x2={tip.px} y2={Hh - padB} stroke={C.accent} strokeWidth="1" opacity="0.55" />
            <circle cx={tip.px} cy={tip.py} r="4.5" fill={C.accent} stroke={C.bg} strokeWidth="1.5" />
            ...
            <text x={tip.tx + tip.tw / 2} y={tip.ty + 32} fill={C.text} fontSize="15" fontWeight="700" fontFamily="'Saira Condensed',sans-serif" textAnchor="middle">${tip.val}</text>
          </g>
        )}
      </svg>
    </div>
